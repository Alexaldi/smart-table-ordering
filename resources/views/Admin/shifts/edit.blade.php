    ...
    <div class="sto-form-page">
        <div class="sto-child-nav">
            <ol class="sto-breadcrumb">
                ...
                <li><span class="active">Edit Shift</span></li>
            </ol>
        ...
        <div class="sto-page-head">
            ...
            <h1 class="sto-page-title">Edit Shift</h1>
        </div>

        <form method="POST" action="{{ route('shifts.update', $shift) }}" class="sto-card">
            @csrf
            @method('PUT')
            ...
            <div class="sto-grid">
                <div class="sto-field full">
                    ...
                    <input id="name" type="text" name="name" value="{{ old('name', $shift->name) }}" class="sto-control @error('name') is-invalid @enderror">
                    @error('name') <div class="sto-error">{{ $message }}</div> @enderror
                </div>

                <div class="sto-field">
                    <label for="start_time" class="sto-label">Jam Mulai</label>
                    <input id="start_time" type="time" name="start_time" value="{{ old('start_time', substr($shift->start_time, 0, 5)) }}" class="sto-control @error('start_time') is-invalid @enderror">
                    @error('start_time') <div class="sto-error">{{ $message }}</div> @enderror
                </div>

                <div class="sto-field">
                    <label for="end_time" class="sto-label">Jam Selesai</label>
                    <input id="end_time" type="time" name="end_time" value="{{ old('end_time', substr($shift->end_time, 0, 5)) }}" class="sto-control @error('end_time') is-invalid @enderror">
