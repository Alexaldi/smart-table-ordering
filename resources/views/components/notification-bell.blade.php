<div
    class="sto-notification"
    data-sto-notification
...
@once
    <style>
            .sto-notification { position: relative; flex: 0 0 auto; }
            .sto-notification-toggle { width: 36px; height: 36px; border: 1px solid #d1d5db; border-radius: 8px; background: #f9fafb; color: #374151; display: inline-flex; align-items: center; justify-content: center; position: relative; cursor: pointer; }
            .sto-notification-toggle:hover, .sto-notification-toggle:focus { background: #eff6ff; border-color: #93c5fd; color: #1d4ed8; }
            .sto-notification-badge { min-width: 18px; height: 18px; border-radius: 999px; background: #dc2626; color: #fff; border: 2px solid #fff; display: inline-flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 800; line-height: 1; padding: 0 4px; position: absolute; top: -7px; right: -7px; }
            .sto-notification-panel { width: min(340px, calc(100vw - 32px)); max-height: 420px; overflow: hidden; background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; box-shadow: 0 18px 42px rgba(15,23,42,.16); position: absolute; top: calc(100% + 10px); right: 0; z-index: 1000; }
            .sto-notification-head { min-height: 48px; display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid #f3f4f6; padding: 0 12px; }
            .sto-notification-head strong { color: #111827; font-size: 14px; }
            .sto-notification-read-all { border: 0; background: transparent; color: #2563eb; cursor: pointer; font-size: 12px; font-weight: 800; padding: 6px 0; }
            .sto-notification-list { max-height: 316px; overflow-y: auto; }
            .sto-notification-item { width: 100%; border: 0; border-bottom: 1px solid #f3f4f6; background: #fff; display: grid; grid-template-columns: 8px minmax(0, 1fr); gap: 10px; padding: 12px; text-align: left; cursor: pointer; }
            .sto-notification-item:hover, .sto-notification-item:focus { background: #f9fafb; }
            .sto-notification-item.unread { background: #eff6ff; }
            .sto-notification-dot { width: 8px; height: 8px; border-radius: 999px; background: #d1d5db; margin-top: 5px; }
            .sto-notification-item.unread .sto-notification-dot { background: #2563eb; }
            .sto-notification-copy { min-width: 0; }
            .sto-notification-copy strong { display: block; color: #111827; font-size: 13px; font-weight: 700; line-height: 1.45; }
            .sto-notification-copy small { display: block; color: #6b7280; font-size: 11px; margin-top: 4px; }
            .sto-notification-empty { color: #6b7280; font-size: 13px; padding: 18px 12px; text-align: center; }
            .sto-sound-enable { position: fixed; right: 18px; bottom: 18px; z-index: 1200; min-height: 40px; border: 1px solid #bfdbfe; border-radius: 10px; background: #eff6ff; color: #1d4ed8; box-shadow: 0 12px 28px rgba(15,23,42,.18); display: inline-flex; align-items: center; gap: 8px; padding: 0 14px; font-size: 13px; font-weight: 800; cursor: pointer; }
            .sto-sound-enable:hover, .sto-sound-enable:focus { background: #dbeafe; border-color: #93c5fd; }
            ...
            @media (max-width: 575.98px) {
                ...
                .sto-sound-enable { left: 12px; right: 12px; justify-content: center; }
            }
        </style>
            ...
            (function () {
                ...
                function setupNotification(root) {
                    ...
                    function showNotificationToast(notifications) {
                        if (!notifications.length || !window.Swal) return;
