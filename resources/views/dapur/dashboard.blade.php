...
<body class="kitchen-dashboard">
    ...
    <main class="kt-shell">
        <div class="kt-page">
                ...
                <div class="kt-user-panel">
                    <div class="kt-shift-pill">
                        ...
                        @if ($dapurShift)
                            <span>
                                {{ $dapurShift->name }}
                                {{ substr($dapurShift->start_time, 0, 5) }} -
                                {{ substr($dapurShift->end_time, 0, 5) }}
                            </span>
                        @else
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="mb-0">
                        @csrf
                        <button type="submit" class="kt-logout">
            ...
            <section class="kt-stats" id="dapurStats">
                @include('dapur.partials.stats', ['stats' => $stats])
            </section>

            <section class="kt-panel">
                <div class="kt-panel-header">
                    <h2 class="kt-panel-title">
                        <i class="fe fe-list"></i> Kitchen Queue
                    </h2>
                    <span class="kt-panel-meta">Current active shift</span>
                </div>

                <div class="kt-orders" id="dapurOrders" data-realtime-url="{{ route('dapur.dashboard.realtime') }}">
                    @include('dapur.partials.orders', ['orders' => $orders])
                </div>
            </section>
        </div>
    </main>
    ...
    <script>
        ...
        function bindDapurDashboardActions() {
            bindKitchenAction('.btn-prepare-order', 'Mulai Proses Order?', 'Ya, Proses', 'Diproses');
        }

        bindDapurDashboardActions();
    </script>

    <script>
        let dapurRefreshTimer = null;
        let dapurRefreshRunning = false;

        async function refreshDapurDashboard() {
            clearTimeout(dapurRefreshTimer);

            dapurRefreshTimer = setTimeout(async function() {
                if (dapurRefreshRunning) {
                    return;
                }

                dapurRefreshRunning = true;

                const ordersWrapper = document.getElementById('dapurOrders');
                const statsWrapper = document.getElementById('dapurStats');

                if (!ordersWrapper || !statsWrapper) {
                    dapurRefreshRunning = false;
                    return;
                }
                ...
                try {
                    ...
                    if (typeof bindDapurDashboardActions === 'function') {
                        bindDapurDashboardActions();
                    }
                } catch (error) {
                    console.error('Failed to refresh kitchen dashboard:', error);
                } finally {
                    dapurRefreshRunning = false;
                }
            }, 300);
